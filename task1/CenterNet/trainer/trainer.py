...
def calcIoUMetric(prec, rec, detects, data, h, w):
    return 0, 0
...
class Trainer(object):
    ...
    def write_log(self, avg_loss, cls_loss=None, iou_loss=None, similarity_loss=None, prec=None, rec=None, mode='TRAIN'):
        ...
        if similarity_loss is not None: log += f'  features_loss: %.3f' % (similarity_loss)
        ...
        if cls_loss is not None:
            if "eval" in mode:
                writer_test.add_scalar('classification loss', cls_loss, (self.start_step))
            else:
                writer_train.add_scalar('classification loss', cls_loss, (self.start_step))

        if iou_loss is not None:
            if "eval" in mode:
                writer_test.add_scalar(' iou loss', iou_loss, (self.start_step))
            else:
                writer_train.add_scalar(' iou loss', iou_loss, (self.start_step))

        if "eval" in mode:
            writer_test.add_scalar('avg loss', avg_loss, (self.start_step))
        else:
            writer_train.add_scalar('avg loss', avg_loss, (self.start_step))

        if similarity_loss is not None:
            if "eval" in mode:
                writer_test.add_scalar(' features_loss', similarity_loss, (self.start_step))
            else:
                writer_train.add_scalar(' features_loss', similarity_loss, (self.start_step))

        if "eval" in mode:
            writer_test.add_scalar(' lr', self.get_lr(), (self.start_step))
        else:
            writer_train.add_scalar(' lr', self.get_lr(), (self.start_step))


        if "eval" in mode:
            writer_test.add_scalar(' prec', prec, (self.start_step))
        else:
            writer_train.add_scalar(' prec', prec, (self.start_step))

        if "eval" in mode:
            writer_test.add_scalar(' rec', rec, (self.start_step))
        else:
            writer_train.add_scalar(' rec', rec, (self.start_step))
    ...
    def train_one_epoch(self):
        ...
        for step, (gt) in enumerate(self.train_loader):
            ...
            self.start_step += 1
            _, _, h, w = gt[0].shape
            prec, rec = calcIoUMetric(prec, rec, pred, gt, h, w)

            if step % self.config.log_interval == 0:
                self.write_log(total_loss / (step + 1), losses[0].item(), losses[1].item(),
                               losses[2].item(), prec / num, rec / num)
    ...
    def val_one_epoch(self):
        ...
        prec, rec = prec / num, rec / num
        self.write_log(total_loss / (step + 1), losses[0].item(), losses[1].item(), losses[2].item(), prec, rec, mode='eval')
        return total_loss / (step + 1)
